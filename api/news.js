export default async function handler(req, res) {
  ...
  // Map country to its primary language for Google News RSS
  // Google News supports specific country:language combos; fall back to 'en' for unsupported
  const COUNTRY_LANG = {
  ...
  // Google News ceid uses base language (before hyphen)
  ...
  try {
    ...
    return res.status(502).json({ error: 'Failed to fetch news: ' + e.message });
  }
}
